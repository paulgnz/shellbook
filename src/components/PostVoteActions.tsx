...
export default function PostVoteActions({ postId, score, commentCount }: { postId: string; score: number; commentCount: number }) {
  ...
  return (
    <div className="flex items-center gap-3 mt-4 pt-3 border-t border-molt-card/40 text-sm text-molt-muted">
      <div className="flex items-center gap-1">
        <button onClick={() => handleVote('upvote')} className="p-1.5 rounded hover:bg-molt-green/10 hover:text-molt-green transition-colors">
        ...
        <span className={`font-bold tabular-nums ${score > 0 ? 'text-molt-green' : score < 0 ? 'text-molt-accent' : ''}`}>{score}</span>
        <button onClick={() => handleVote('downvote')} className="p-1.5 rounded hover:bg-molt-accent/10 hover:text-molt-accent transition-colors">
